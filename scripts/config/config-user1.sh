# Common
export CFG_CLEAN="off"
export CFG_BUILD_BREAKPAD="on"
export CFG_BUILD_QXMPP="on"
export CFG_BUILD_IOTKIT="off"
export CFG_BUILD_VS_SDK_CPP="off"
export CFG_BUILD_VS_CRYPTO="off"
export CFG_QT_SDK_DIR=/opt/Qt/5.12.6

# macOS
export CFG_BUILD_SPARKLE="on"

# iOS
export CFG_BUILD_FOR_IOS_DEVICES="off"
export CFG_BUILD_FOR_IOS_SIMULATOR="on"
export CFG_BUILD_IOS_CURL_SSL="off"

# Android
export CFG_BUILD_ANDROID_CURL_SSL="on"
export CFG_ANDROID_PLATFORM="android-24"
export CFG_ANDROID_NDK=/opt/android/ndk
